...
from vae_model import VAE, vae_loss, get_flat_params, set_flat_params, average_weights
...
def main():
    ...
    csv_path = f"generative_results/fedavg_vae_{args.dataset}_{args.availability}_{args.split}_seed{args.seed}.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["round", "loss", "recon_loss", "kl_loss", "num_available"])
        ...
        for r in range(1, args.rounds + 1):
            ...
            for cid in available:
                w = local_train(model, client_loaders[cid], global_weights, device, args.lr, args.local_epochs)
            ...
            if r % args.eval_freq == 0 or r == 1:
                ...
                writer.writerow([r, loss, rec, kl, len(available)])
                print(f"FedAvg-VAE round {r} loss {loss:.4f} recon {rec:.4f} kl {kl:.4f} available {len(available)}")

    set_flat_params(model, global_weights, device)
    model.eval()
    with torch.no_grad():
        z = torch.randn(64, args.latent_dim).to(device)
        save_image(model.decode(z).cpu(), f"generative_results/fedavg_vae_samples_seed{args.seed}.png", nrow=8)
